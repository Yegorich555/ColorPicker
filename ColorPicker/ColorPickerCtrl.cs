...
namespace ColorPicker
{
    public partial class ColorPickerCtrl : UserControl
    {
        ...
        string HslInfo { get => m_colorWheel.SelectedHSLColor.ToStringWeb((int)Math.Floor(255f * m_opacity)).ToLower() + ";"; }
        string RgbInfo
        {
            get
            {
                var c = Color.FromArgb((int)Math.Floor(255f * m_opacity), m_selectedColor);
                return "rgb" + (c.A == 255 ? "" : "a") + $"({c.R}, {c.G}, {c.B}" + (c.A == 255 ? "" : ", " + (c.A / 255f).ToString("f2")) + ");".ToLower();
            }
        }
        string HexInfo
        {
            ...
            {
                var c = Color.FromArgb((int)Math.Floor(255f * m_opacity), m_selectedColor);
                var str = ColorTranslator.ToHtml(c);
                if (c.A != 255)
                    str += c.A.ToString("X2");
                return str.ToLower() + ";";
            }
        ...
        void UpdateInfo() => m_infoLabel.Text = $"{HexInfo}\n{RgbInfo}\n{HslInfo}";
