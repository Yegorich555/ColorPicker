...
namespace ColorPicker
{
    public struct HSLColor
    {
        ...
        public override string ToString() => string.Format("HSL({0:f2}, {1:f2}, {2:f2})", Hue, Saturation, Lightness);

        public string ToStringWeb(int alphaChannel = 255)
        {
            var str = new StringBuilder("hsl");
            if (alphaChannel != 255)
                str.Append('a');
            str.Append('(');
            str.Append(Convert.ToInt16(Hue)); str.Append(", ");
            str.Append(Convert.ToInt16(Saturation * 100)); str.Append("%, ");
            str.Append(Convert.ToInt16(Lightness * 100)); str.Append("%");

            if (alphaChannel != 255)
                str.AppendFormat(", {0:f2}", alphaChannel / 255f);
            str.Append(')');
